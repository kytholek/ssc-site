...
for ($i = 0; $i -lt $articles.Count; $i++) {
  ...
  $prevButton = if ($prevArticle) {
    "    <a class=`"post-nav-btn prev`" href=`"/blog/$($prevArticle.dir)/`">`n" +
  ...
  $nextButton = if ($nextArticle) {
    "    <a class=`"post-nav-btn next`" href=`"/blog/$($nextArticle.dir)/`">`n" +
  ...
  $newNav = "  <div class=`"post-nav`">`n$prevButton`n$nextButton`n  </div>"
  
  # Read the file
  $content = Get-Content $filePath -Raw
  
  # Replace the post-nav section
  $content = $content -replace '<div class="post-nav">[\s\S]*?</div>', $newNav
...
Write-Host "All 29 blog posts updated with prev/next navigation!"
