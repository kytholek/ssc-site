...
<head>
  ...
  <style>
    ...
    .post-hero-glyph { font-family: "Cinzel Decorative", serif; font-size: 72px; color: #c9a84c; margin-bottom: 16px; display: block; text-shadow: 0 0 60px #c9a84c55; }
...
<div class="post-wrap">
  ...
  <!-- Hero -->
  <div class="post-hero">
    <div class="post-hero-eyebrow">The System</div>
    <h1>{HERO_TITLE}</h1>
  </div>
...
foreach ($slug in $articlesToConvert) {
  ...
  # Extract the blog-post div and its content
  if ($existingContent -match '(?s)<div class="blog-post"[^>]*>(.*?)</div>\s*(?:</div>)?(?:\s*<div class="related-posts">.*?</div>)?(?:\s*<footer>.*?</footer>)?(?:\s*</body>)?(?:\s*</html>)?$') {
    $postContent = $matches[1]
    
    # Extract data attributes for title/description
    if ($existingContent -match 'data-title="([^"]*)"') {
      $title = $matches[1]
      $heroTitle = $title
    } else {
      $heroTitle = $slug -replace '-', ' ' | Get-Culture | ForEach-Object { $_.TextInfo.ToTitleCase($_) }
    }
    
    if ($existingContent -match 'data-description="([^"]*)"') {
      $description = $matches[1]
    } else {
      $description = "Read about $slug in Simulation Source Code."
    }
    ...
    $newHtml = $htmlTemplate `
      ...
      -replace '{CONTENT_PLACEHOLDER}', $postContent
    
    Set-Content -Path $path -Value $newHtml
    Write-Host "✓ Converted: $slug"
    $converted++
  } else {
    Write-Host "✗ Failed: $slug (could not extract content)"
  }
}
