...
<head>
  ...
  <style>
    ...
    @media (max-width: 640px) {{ .post-nav {{ grid-template-columns: 1fr; }} .post-hero-glyph {{ font-size: 52px; }} }}
  </style>
</head>
<body>
<nav>
...
foreach ($dir in $dirs) {
    ...
    $finalHtml = $htmlTemplate -replace '{DIR}', $dir `
                               -replace '{TITLE}', $title `
    ...
    # Save it back
    Set-Content -Path $filePath -Value $finalHtml -Encoding UTF8
    Write-Host "✓ Converted $dir"
}

Write-Host "`nDone! All 20 blog posts have been converted to standalone HTML pages with CSS."
